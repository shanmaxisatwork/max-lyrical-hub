...
def main():
    ...
    # Telegram summary
    lines = [f"✅ <b>Monitor Complete!</b>\n📊 Found <b>{len(top_videos)} long videos</b> queued for upload:\n"]
    for v in queue_to_add:
        mins = v["duration_s"] // 60
        lines.append(
            f"#{v['rank']} — <b>{v['title'][:50]}</b>\n"
            f"   📺 {v['channel_name']} | ⏱ {mins}m | 👁 {v['views']:,}\n"
            f"   🔗 https://youtube.com/watch?v={v['id']}\n"
        )
    lines.append("⬇️ Download + Processing will start in the afternoon run!")
    telegram("\n".join(lines))
